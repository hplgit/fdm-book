"""Illustration of staggered mesh in 1D space and time."""
...
for i, t in enumerate(t_mesh):
    ...
    if 0 <= i < len(t_mesh)-1:
        ...
        v_points[i] = Composition(dict(
            ...
            v_point=Text(r'$v_{%d/2}^{n+\frac{1}{2}}$' % (2*i+1),
...
axes = Composition(dict(
    x=Axis(point(0,-b), t_mesh[-1] + 0.2*t_axis_extent, '$x$',
