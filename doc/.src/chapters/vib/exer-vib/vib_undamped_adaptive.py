...
# u and t will only consist of [I, u^Nt] and [0,T], i.e. 2 values each, while
# solver.u_all and solver.t_all contain all computed points.
# solver.u_all is a list with arrays, one array (with 2 values) for
# each point in time.
u_adaptive = np.array(solver.u_all)

# For comparison, we solve also with simple FDM method
Nt_simple = len(solver.t_all)   # get no of time steps from adaptive method
t_simple = np.linspace(0, T, Nt_simple+1)   # create equally spaced steps
...
for n in range(1, Nt_simple):
    u_simple[n+1] = 2*u_simple[n] - u_simple[n-1] - dt**2*w**2*u_simple[n]
