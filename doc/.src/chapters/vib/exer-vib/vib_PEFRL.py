...
def compute_orbit_and_error(f, solver_ID, 
                            timesteps_per_period=20, 
                            grouped_orbits=10,
                            N_grouped_orbits=1000):
    '''
    For one particular solver:
    Calculte the orbit for a multiple of grouped orbits, i.e.
    N_grouped_orbits*grouped_orbits...
...
def orbit_error_with_dt_10000_years_fixed(f_EC, f_RK4, g, solvers):
    '''
    With each solver in list "solvers":
    Simulate 10 000 orbits 10 times, each time with a different dt.
    ...
    for solver_ID in solvers:
        ...
        dt_values = []
        cpu_values = []
        #timesteps_per_period = 800       
        timesteps_per_period = 3000       
        for i in range(10):            
        ...
        p = np.polyfit(dt_values, E_values, 4)
        p_str = map(str, p)
...
def orbit_error_with_years_dt_fixed(f_EC, f_RK4, g, solvers):
    '''
    With each solver in the list "solvers":
    Simulate 100 000 orbits with a fixed dt corresponding to 1000 
    steps per cycle (year). Collect 2D position error for 
    each 1000'th run, plot these errors and CPU. Finally, make an
    empirical formula for error and CPU as functions of number 
    of cycles. 
    '''
    ...
    for solver_ID in solvers:
        ...
        plt.plot(years, 
                 E, 'b*',
                 years, 
                 E_of_years(years, p[0], p[1], p[2], p[3], p[4]), 'r--')
        plt.xlabel('Number of years')
...
def compute_orbit_error_and_CPU(): 
    ...
    solvers = ['EC', 'RK4', 'PEFRL']
        
    print 'Find orbit error after 10 000 years as fu. of dt...'
    orbit_error_with_dt_10000_years_fixed(f_EC, f_RK4, g, solvers)

    print 'Compute orbit error as fu. of no of years (fixed dt)...'
    orbit_error_with_years_dt_fixed(f_EC, f_RK4, g, solvers)
