...
def solver(I, U0, v, L, dt, C, T, user_action=None,
    ...
    integral[0] = dx*(0.5*u_1[0] + 0.5*u_1[Nx] + np.sum(u_1[1:-1]))
    ...
    for n in range(0, Nt):
        if scheme == 'FE':
            if periodic_bc:
                i = 0
                u[i] = u_1[i] - 0.5*C*(u_1[i+1] - u_1[Nx])
                u[Nx] = u[0]
                #u[i] = u_1[i] - 0.5*C*(u_1[1] - u_1[Nx])
            for i in range(1, Nx):
                u[i] = u_1[i] - 0.5*C*(u_1[i+1] - u_1[i-1])
        elif scheme == 'LF':
            if n == 0:
...
def run(scheme='FEUS', case='gaussian', C=1):
    ...
    U0 = 0
    dt = 0.01
    #dt = 0.001  # visually exact for C=0.8 and LF
    T = 0.7
    v = 1
...
def solver_theta(I, v, L, dt, C, T, theta=0.5, user_action=None, FE=False):
    ...
    # Time loop
    for n in range(0, Nt):
        b[1:-1] = u_1[1:-1] + 0.5*(1-theta)*C*(u_1[:-2] - u_1[2:])
        if FE:
            b[1:-1] += 1./6*u_1[:-2] + 1./6*u_1[:-2] + 4./6*u_1[1:-1]
        b[0] = u_1[Nx]; b[-1] = u_1[0]  # boundary conditions
...
if __name__ == '__main__':
    ...
    run(scheme='CN', case='gaussian', C=0.8)
